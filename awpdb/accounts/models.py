...
class User(AbstractUser):
    ...
    department = ForeignKey(Department, null=True, related_name='+', on_delete=SET_NULL)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.original_schooldefault = self.schooldefault
    ...
    def save(self, *args, **kwargs):
        ...
        # logger.debug('class User(AbstractUser) save request_user: ' + str(request_user))
    ...
    @property
    def examyear_padlock(self): # PR2018-08-24
        # examyear_padlock only shows when examyear is locked
        _examyear_padlock = False
        if self.examyear:
             _examyear_padlock = self.examyear.locked
        return _examyear_padlock

    @property
    def school(self): # PR2018-09-03
        _school = None
        if self.country and self.examyear and self.schooldefault:
            # get school from this schooldefault and this examyear
            _school= School.objects.filter(schooldefault=self.schooldefault, examyear=self.examyear).first()
        return _school
    ...
    def school(self): # PR2018-09-15
        school = None
        if self.country and self.examyear and self.schooldefault:
            # get school from this schooldefault and this examyear. Countr is field of Examyear, therefore filter Counrry not necessary
            school= School.objects.filter(schooldefault=self.schooldefault, examyear=self.examyear).first()
        return school
    ...
    def schooldefault_choices(self, selected_user):
        ...
        # logger.debug('class User schooldefault_choices  request_user.username: ' + str(self.username))
        # logger.debug('class User schooldefault_choices  request_user.country: ' + str(self.country))

        # logger.debug('class User schooldefault_choices  selected_user.username: ' + str(selected_user.username))
        # logger.debug('class User schooldefault_choices  selected_user.country: ' + str(selected_user.country))
        ...
        if _selecteduser_countryid: # _selecteduser_countryid = False when _selecteduser_countryid == 0 or None:
            _schooldefaults = Schooldefault.objects.filter(country_id=_selecteduser_countryid)
            logger.debug('class User(AbstractUser) _schooldefaults: ' + str(_schooldefaults))
            if _schooldefaults:
                for _schooldefault in _schooldefaults:
                    _schooldefault_str = ''
    ...
    # PR2018-07-31 debug. This is a method, not a @property. Property gave error: 'list' object is not callable
    # see: https://www.b-list.org/weblog/2006/aug/18/django-tips-using-properties-models-and-managers/?utm_medium=twitter&utm_source=twitterfeed
    def department_choices(self, selected_user):
        # PR2018-06-02 this function is used in UserEditForm
        # PR2018-08-29 Show only departments from dep_list of selected_user.schooldefault
        # self = request_user, not selected_user when called by UserEditForm

        logger.debug('class User department_choices  request_user.username: ' + str(self.username))
        logger.debug('class User department_choices  request_user.schooldefault: ' + str(self.schooldefault))
        logger.debug('class User department_choices  selected_user.username: ' + str(selected_user.username))
        logger.debug('class User department_choices  selected_user.schooldefault: ' + str(selected_user.schooldefault))

        self.schooldefault.dep_list
    ...
    def may_add_or_edit_users(self):
        ...
        if self.is_perm_admin:
            if self.is_role_system:
                ...
                if self.country is not None:
                    if self.schooldefault is not None:
                        _has_permit = True
        return _has_permit
...
# -- subjects: same as scheme PR2018-09-02
    @property
    def enable_subjects_view(self):
        return not bool(self.message('scheme_etc_view'))

    @property
    def message_subjects_view(self):
        return self.message('scheme_etc_view')

    @property
    def enable_subjects_modify(self):
        return not bool(self.message('scheme_etc_modify'))

    @property
    def message_subjects_modify(self):
        return self.message('scheme_etc_modify')
...
# -- schools
    ...
    @property
    def enable_schools_edit(self):  # PR2018-09-15
        return not bool(self.message('school_edit'))

    @property
    def message_schools_edit(self):  # PR2018-09-15
...
# =====================================================================
    # - userlist, but only by role_insp
        if page_name == 'user_view':
            ...
            elif not self.schooldefault:  # role_school need school selected to view userlist
...
# - schooldefault / subjectdefault / departments / levels / sectors:  can only be viewed by role_system and role_insp
...
# - schooldefault / subjectdefault / departments / levels / sectors: can only be modified by role_system and role_insp, only admin
        # TODO exclude read, authorize and None permissions
        if page_name == 'default_items_modify':
            if not self.is_role_insp_or_system_and_perm_admin:
                ...
                return None  # Not disabled: admin of role_system and role_insp can modify default schools, if country not locked
...
# =====================================================================
        ...
        if page_name == 'scheme_etc_modify':
            # TODO exclude read, authorize and None permissions
            if not self.is_role_insp_or_system_and_perm_admin:
                return _("You don't have permission to modify these items.")
            elif self.country_locked:
 ...
 # =====================================================================
# ===== no permissions if no schooldefault selected
# =====================================================================
        if not self.schooldefault:
            return _("You are not connected to a school. You cannot view this page.")
# =====================================================================
